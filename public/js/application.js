...
var showDirectories = function showDirectories(directoriesList) {
    ...
    $.each(directoriesList, function (index, directory) {
        ...
        })).append($('<span />').addClass('form-check-sign').append($('<span />').addClass('check')))))).append($('<td />').addClass('td-type text-left').append($('<i />').addClass('material-icons text-gray text-center').text('folder'))).append($('<td />').addClass('td-type text-left').append($('<i />').addClass('material-icons text-gray text-center').text(directory.locked == 'lock' ? 'lock' : ''))).append($('<td />').append($('<a />').addClass('text-gray change-dir').attr('href', directory.path).text(directory.name))).append($('<td />').addClass('url').text('')).append($('<td />').addClass('td-size text-center').text('')).append($('<td />').addClass('td-modified text-center').text('')).append($('<td />').addClass('td-actions text-right').append($('<button />').addClass('btn btn-info btn-sm ml-2 tooltip-btn info-btn').attr('type', 'button').text(' Show Info').prepend($('<i />').addClass('material-icons').text('info'))).append($('<button />').addClass('btn btn-warning btn-sm ml-2 tooltip-btn rename-btn').attr('type', 'button').text('Rename').prepend($('<i />').addClass('material-icons').text('question_answer')))));
    });
};

var showFiles = function showFiles(filesList) {
    ...
    $.each(filesList, function (index, file) {
        ...
        $('#content_list tbody').append($('<tr />').append($('<td>').addClass('td-check').append($('<div />').addClass('form-check').append($('<label />').addClass('form-check-label').append($('<input />').addClass('form-check-input').attr({
            type: 'checkbox',
            value: file.path
        })).append($('<span />').addClass('form-check-sign').append($('<span />').addClass('check')))))).append($('<td />').addClass('td-type text-left file-visibility').append($('<i />').addClass('material-icons text-gray text-center').text(file.public_status == 'public' ? 'cloud_queue' : 'cloud_off'))).append($('<td />').addClass('td-type text-left lock-status').append($('<i />').addClass('material-icons text-gray text-center').text(file.lock_status == 2 ? 'lock' : file.lock_status == 1 ? 'delete_forever' : 'lock_open'))).append($('<td />').addClass().text(file.name)).append($('<td />').addClass('td-url').text('')).append($('<td />').addClass('td-size text-center').text('')).append($('<td />').addClass('td-modified text-center').text('')).append($('<td />').addClass('td-actions text-right').append($('<button />').addClass('btn btn-primary btn-sm ml-2 tooltip-btn edit-btn').attr('id', 'btn_edit').text(' Edit').prepend($('<i />').addClass('material-icons').text('edit'))).append($('<button />').addClass('btn btn-success btn-sm ml-2 tooltip-btn view-btn').attr('id', 'btn_view').text(' View').prepend($('<i />').addClass('material-icons').text('visibility'))).append($('<div />').addClass('dropdown').append($('<button />').addClass('btn dropdown-toggle').attr('id', 'more-action').attr('data-toggle', 'dropdown').attr('aria-haspopup', 'true').attr('aria-expanded', 'false').text(' More').prepend($('<i />').addClass('material-icons').text('apps'))).append($('<div />').addClass('dropdown-menu dropdown-info').append($('<a />').addClass('dropdown-item visibility-btn').attr('id', 'btn_visibility').text(file.public_status == 'public' ? ' Make Private' : ' Make Public').prepend($('<i />').addClass('material-icons').text(file.public_status == 'public' ? 'cloud_off' : 'cloud_queue'))).append($('<a />').addClass('dropdown-item delete-lock-btn').attr('id', 'btn_lock_edit').text(' Lock Delete').prepend($('<i />').addClass('material-icons').text('delete_forever'))).append($('<a />').addClass('dropdown-item edit-lock-btn').attr('id', 'btn_lock_edit').text(' Lock Edit').prepend($('<i />').addClass('material-icons').text('lock'))).append($('<a />').addClass('dropdown-item download-btn').text(' Download').prepend($('<i />').addClass('material-icons').text('save_alt'))).append($('<a />').addClass('dropdown-item info-btn').text(' Show Info').prepend($('<i />').addClass('material-icons').text('info'))).append($('<a />').addClass('dropdown-item rename-btn').text(' Rename').prepend($('<i />').addClass('material-icons').text('question_answer')))))));
    });
};

var showNavigation = function showNavigation(navigationList) {

    $('#quick_navigation .breadcrumb').empty();

    $.each(navigationList, function (index, directory) {

        $('#quick_navigation .breadcrumb').append($('<li />').addClass('breadcrumb-item').append($('<a />').addClass('change-dir text-primary font-weight-bold').attr('href', directory.path).text(directory.name)));
    });
};

var showInfo = function showInfo(dirInfo) {
...
var makeDirectory = function makeDirectory() {
    ...
    var form = $('#make_directory_frm');

    var currentDirectory = $('#current_directory').data('path');

    var directoryName = $('input[name="directory"]', form).val();
    ...
    axios.post('/api/make-directory', qs.stringify({
...
var renameContent = function renameContent(path) {

    var form = $('#rename_content_frm');
    ...
    axios.post('/api/rename', qs.stringify({
        'path': path,
        'name': name
    })).then(function () {
...
var removeContent = function removeContent() {
    ...
    axios.post('/api/remove', qs.stringify({
...
var pasteContent = function pasteContent() {

    var pasteContent = JSON.parse(sessionStorage.getItem('paste_content'));
    ...
    axios.post('/api/paste', qs.stringify({
...
var uploadFiles = function uploadFiles() {
    ...
    axios.post('/api/upload', requestData).then(function () {
...
$(document).ready(function () {
    ...
    $('#search_form').on('submit', function (event) {
        ...
        var search = $('input[type="text"]', $(this)).val().toLowerCase();
    ...
    $(document).on('click', '.rename-btn', function (event) {
        ...
        showEdit('Rename', renameContentForm.get(0)).then(function (value) {
    ...
    $(document).on('click', '.download-btn', function (event) {
        ...
        var path = $(this).parents('tr').find('input[type="checkbox"]').val();
    ...
    $('#make_directory_btn').on('click', function (event) {
        ...
        var makeDirectoryForm = $('<form />').attr('id', 'make_directory_frm').append($('<div />').addClass('form-group').append($('<input />').addClass('form-control').attr({
            'type': 'text',
            'name': 'directory',
            'autocomplete': 'off',
        ...
        showEdit('Make Directory', makeDirectoryForm.get(0)).then(function (value) {
    ...
    $('#paste_btn').on('click', function (event) {
        ...
        showConfirmation('You are trying to paste content.').then(function (value) {
    ...
    $('#remove_btn').on('click', function (event) {
        ...
        showConfirmation('You are trying to delete content.').then(function (value) {
    ...
    $('#upload_file_btn').on('click', function (event) {
        ...
        var uploadFileForm = $('<form />').attr({
            enctype: 'multipart/form-data',
            id: 'upload_files_frm'
        }).append($('<div />').addClass('form-group').append($('<input />').addClass('form-control-plaintext').attr('type', 'text').val('Select files for uploading or drag them directly to the files list'))).append($('<div />').addClass('form-group').append($('<input />').addClass('form-control').attr({
            type: 'text',
            name: 'local_file',
            placeholder: 'Click to select files',
            autocomplete: 'off'
        })).append($('<input />').addClass('d-none').attr({
            type: 'file',
            id: 'local_file',
            multiple: true
        })));

        showEdit('Upload Files', uploadFileForm.get(0)).then(function (value) {
    ...
    $('.content').on('dragenter', function (event) {
        ...
        $('body').append($('<div />').addClass('drop-container process-container').html('Drop files to start upload'));
    });
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_sweetalert__ = __webpack_require__(15);
...
// Errors
window.showFormError = function (message, target) {
    ...
    if (target.next('.invalid-tooltip').length === 0) {
        target.after($('<div />').addClass('invalid-tooltip'));
    }

    target.next('.invalid-tooltip').html(message);
};

window.clearFormError = function (target) {
...
window.processErrors = function (error, form) {
    ...
    if (error.response.status == 422) {
        ...
        $.each(errors, function (field, message) {
            ...
            var formControll = $('input[name="' + field + '"]', form);
...
window.showEdit = function (title, form) {
...
// Spinner
window.showSpinner = function (target) {

    target.prop('disabled', true);

    target.append($('<div />').addClass('spinner-container').append($('<i />').addClass('material-icons spinner').html('autorenew')));
};

window.hideSpinner = function (target) {
...
window.startProcess = function () {
    ...
    $('body').append($('<div />').addClass('process-container').append($('<i />').addClass('material-icons spinner').html('autorenew')));
};
